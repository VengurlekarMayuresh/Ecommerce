...
if (-not (Get-Command az -ErrorAction SilentlyContinue)) {
    Write-Host "[ERROR] Azure CLI is not installed. Install from https://aka.ms/azure-cli" -ForegroundColor Red
    exit 1
}
...
# 1. Cosmos DB
Write-Host "[1/9] Creating Cosmos DB..." -ForegroundColor Cyan
az cosmosdb create `
...
Write-Host "[NOTE] Configure email domain in Azure Portal" -ForegroundColor Yellow
...
Write-Host "[NOTE] Configure FCM credentials in Azure Portal" -ForegroundColor Yellow
...
# Email sender
$SenderEmail = Read-Host "Email sender address (use temp@temp.com for now, configure later)"
